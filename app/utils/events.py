...
class EventManager:
    ...
    async def subscribe(self, user_id: int):
        ...
        logger.info(f"[SSE] User {user_id} subscribed. Active connections: {len(self.user_queues[user_id])}")
    ...
    async def broadcast(self, user_id: int, event_name: str, data: str):
        ...
        if user_id in self.user_queues:
            logger.info(f"[SSE] Broadcasting event '{event_name}' to user {user_id} ({len(self.user_queues[user_id])} connections)")
            message = f"event: {event_name}\ndata: {data}\n\n"
            for queue in self.user_queues[user_id]:
                await queue.put(message)
        else:
            logger.debug(f"[SSE] No active connections for user {user_id} to broadcast '{event_name}'")
